  ...
  useEffect(() => {
    ...
    const logger: Logger = {
      ...
      system: (text) => {
        ...
        if (
          lowerText.startsWith("connecting to") ||
          lowerText.startsWith("connected to")
        ) {
          return;
        }
        ...
        <div className="max-w-3xl mx-auto w-full flex flex-col gap-[10px]">
          {messages.map((msg) => (
            ...
            >
              ...
              {msg.role === "agent" && (
                <div className="w-full text-[14px] leading-relaxed break-words text-[#cccccc] [&>p]:mb-[8px] last:[&>p]:mb-0 [&>ul]:list-disc [&>ul]:ml-5 [&>ul]:mb-[8px] [&>ol]:list-decimal [&>ol]:ml-5 [&>ol]:mb-[8px] [&>pre]:bg-[#252526] [&>pre]:p-3 [&>pre]:rounded-md [&>pre]:mb-[8px] [&>code]:bg-[#252526] [&>code]:px-1 [&>code]:rounded">
